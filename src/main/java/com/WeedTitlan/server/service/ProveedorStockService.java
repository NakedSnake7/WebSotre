...
public class ProveedorStockService {
    ...
    public Map<String, List<ProductoStock>> obtenerStockPorMarca() {
        ...
        try {
            if (credentialsPath.isBlank() || spreadsheetId.isBlank()) {
                System.out.println("⚠ Google Sheets deshabilitado: credenciales o spreadsheet ID no configurados.");
                return inventarioPorMarca;
            }

            InputStream credentialsStream = getClass().getResourceAsStream("/credenciales.json");
            if (credentialsStream == null) {
                throw new RuntimeException("No se pudo encontrar credenciales.json en classpath");
            }
            ...
            for (String rango : RANGOS_TABLAS) {
                ...
                // La primera fila tiene la marca
