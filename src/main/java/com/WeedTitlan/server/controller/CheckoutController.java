	        ...
	        try {
	         ...
	         // Crear la orden
	            ...
	            if (inputStream == null) {
	                ...
	                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error: No se pudo cargar la plantilla de email.");
	            }
	            String template = new String(inputStream.readAllBytes(), StandardCharsets.UTF_8);

	            // Reemplazar datos dinámicos en la plantilla
	            String emailHTML = template
	                .replace("{NOMBRE}", user.getName() != null ? user.getName() : "Cliente")
